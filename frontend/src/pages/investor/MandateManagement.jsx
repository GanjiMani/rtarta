...
export default function MandateManagement() {
  ...
  const activate = async (id) => {
    ...
    setSuccessMsg("");
    try {
      const res = await fetchWithAuth(`/api/investor/mandates/verify/${id}`, {
        method: "PUT",
      });
      if (!res.ok) {
        const errorData = await res.json();
      ...
      setSuccessMsg("Mandate activated successfully!");
      setTimeout(() => setSuccessMsg(""), 5000);
      // Refresh the mandates list
      fetchMandates();
    } catch (err) {
      setError(err.message);
      console.error("Error activating mandate:", err);
    }
  };
                  ...
                  {mandates.map((m) => (
                    <tr key={m.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {m.account_number ? `****${m.account_number.slice(-4)}` : "N/A"}
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                            (m.mandate_status === "active" || m.mandate_status === "Active")
                              ? "bg-green-100 text-green-800"
                              : m.mandate_status === "pending" || m.mandate_status === "Pending"
                              ? "bg-yellow-100 text-yellow-800"
                              : "bg-red-100 text-red-800"
                          }`}
                        >
                          {m.mandate_status || "inactive"}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {m.mandate_limit ? `₹${m.mandate_limit.toLocaleString()}` : "N/A"}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {m.created_at
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex items-center justify-end gap-3">
                          {(m.mandate_status === "inactive" || m.mandate_status === "Inactive") && (
                            <button
                          ...
                          {(m.mandate_status !== "revoked" && m.mandate_status !== "Revoked" && m.mandate_status !== "cancelled" && m.mandate_status !== "Cancelled") && (
                            <button
                              onClick={() => revoke(m.id)}
